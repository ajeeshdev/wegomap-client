...
export default function SpecialEventDetailPage() {
    ...
    useEffect(() => {
        const fetchEvent = async () => {
            try {
                // For special events, we prioritize special-events API
                ...
                if (data.success) {
                    ...
                    // Fallback to standard events if not found or mismatch
                    res = await fetch(`${API_URL}/events/${slugOrId}`);
                    data = await res.json();
                    if (data.success) {
                        setEvent(data.data);
                    } else {
                        // Search all if direct match fails
    ...
    return (
        <main className="eventsPage detail-v3">
            <DynamicPageBanner
                ...
                fallbackImage={event.images?.[0] || "https://images.unsplash.com/photo-1511795409834-ef04bbd61622?auto=format&fit=crop&q=80&w=1200"}
                breadcrumbs={[
