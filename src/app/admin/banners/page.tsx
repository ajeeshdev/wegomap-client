      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredData.map((item: any) => (
            <div key={item._id} className="admin-form-card group p-0 overflow-hidden flex flex-col hover:shadow-xl transition-all border-none bg-white shadow-sm h-auto">
              
              <div className="h-48 bg-slate-100 relative overflow-hidden shrink-0 border-b border-slate-100">
                {item.image ? (
                  <img src={item.image} alt="Banner" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                ) : (
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-2 pt-6">
                    <Link href={`/admin/banners/${item._id}/edit`} className="flex-1 flex items-center justify-center gap-2 h-11 bg-slate-900 border border-slate-900 rounded-2xl text-white font-black uppercase tracking-[0.2em] text-[10px] hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 active:scale-95">
