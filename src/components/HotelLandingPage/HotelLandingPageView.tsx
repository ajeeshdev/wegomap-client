...
export default function HotelLandingPageView({ data }: { data: HotelLandingPageContent }) {
  ...
  const [temp, setTemp] = useState<string>("24°C");
  ...
  useEffect(() => {
    ...
    const fetchWeather = async () => {
       ...
       try {
          ...
          if (loc.includes('kochi')) { lat = 9.9312; lon = 76.2673; }
          ...
          if (!res.ok) return; // Silent return if API is down
          const wJson = await res.json();
          if (wJson.current_weather) setTemp(`${Math.round(wJson.current_weather.temperature)}°C`);
       } catch (e) {
          // Weather fetch failed - silent fallback to default temp
      ...
      <section className="hotel-hero-centered" id="home">
        ...
        <div className="hero-content container-ctn text-center">
            <div className="hero-badge-wrap animate-up">
               <span className="hero-badge-text uppercase">{data.hero_subtitle || 'Welcome to Aroma Hills'}</span>
            </div>
            ...
            <div className="hero-action-buttons animate-up delay-4">
               <a href="#rooms" className="btn-primary">{data.hero_cta_text || 'Check Availability'}</a>
