      ...
      <div className="property-edit-layout">
        <div className="tabs-sidebar">
           ...
           <div className="mt-8 pt-8 border-t border-slate-100 px-4">
              ...
              {isEdit && (
                <Link href={`/packages/${formData.slug}`} target="_blank" className="mt-4 flex items-center gap-2 text-[10px] font-bold text-indigo-600 hover:underline uppercase tracking-widest">
                 ...
                 <div className="editor-card">
                    <div className="card-header flex items-center justify-between">
                       ...
                       <div className="flex items-center gap-4">
                          ...
                          <div className="bg-indigo-600 text-white px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest">{formData.package_ids.length} Linked</div>
                       </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       {allPackages
                          ...
                          .map((pkg: any) => {
                             ...
                             return (
                                <div key={pkg._id} onClick={() => {const ni = isSelected ? formData.package_ids.filter(id => id !== pkg._id) : [...formData.package_ids, pkg._id]; setFormData({ ...formData, package_ids: ni });}} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${isSelected ? 'border-indigo-600 bg-indigo-50/50':'border-slate-100 bg-slate-50/30 hover:border-slate-300'}`}>
                                   ...
                                   {isSelected && <ShieldCheck size={14} className="text-indigo-600" />}
                                </div>
                             );
                          })}
                    </div>
