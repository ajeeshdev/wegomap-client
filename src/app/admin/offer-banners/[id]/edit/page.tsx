...
export default function EditOfferBanner() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/offer-banners/${id}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          setFormData({
  ...
  if (fetching) return <div className="p-12 text-center font-bold text-slate-400 uppercase tracking-widest text-[9px]">Loading Config...</div>;
      ...
      <div className="admin-page-header">
        <div className="flex items-center gap-6">
          ...
          <div className="min-w-0">
            ...
            <p className="admin-page-subtitle mt-1 text-slate-400">Update the promotional slider details</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          >
            <ShieldCheck size={20} /> {loading ? 'Compiling...' : 'Commit Changes'}
          </button>
        </div>
      </div>
