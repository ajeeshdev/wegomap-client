...
const DEFAULT_CONTENT: ToursPageContent = {
    banner: {
        title: "Explore All Destinations",
        subtitle: "Browse handpicked packages — from Kerala backwaters to international escapes.",
        preTitle: "Curated Journeys",
        image: ""
    }
};
...
export default function ToursSettingsPage() {
    ...
    const [content, setContent] = useState<ToursPageContent>(DEFAULT_CONTENT);
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/options/bulk`, {
                ...
                body: JSON.stringify({ options: [{ key: 'tours_page_settings', value: JSON.stringify(content) }] })
            });
            const json = await res.json();
            if (json.success) toast.success('Tours page settings updated!');
            ...
            <div className="admin-page-header">
                <div>
                    <h2 className="admin-page-title admin-page-title--section">
                        ...
                        Tours / Packages Page <span className="text-black">Configuration</span>
                    </h2>
                    <p className="admin-page-subtitle text-slate-500 font-bold">Manage the hero banner for the all-packages listing page</p>
                </div>
                ...
                <div className="admin-form-card !p-8">
                    <div className="flex items-center gap-3 mb-8 border-b border-slate-50 pb-6">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400 font-bold">Hero section for /packages (All Tours listing)</p>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-6">
                            <div className="admin-form-group">
                                ...
                                <input type="text" value={content.banner?.preTitle} onChange={e => setContent({ ...content, banner: { ...content.banner, preTitle: e.target.value } })} className="admin-form-input font-bold text-black" placeholder="Curated Journeys" />
                            </div>
                            <div className="admin-form-group">
                                ...
                                <input type="text" value={content.banner?.title} onChange={e => setContent({ ...content, banner: { ...content.banner, title: e.target.value } })} className="admin-form-input font-bold text-black" placeholder="Explore All Destinations" />
                            </div>
                            <div className="admin-form-group">
                                ...
                                <textarea rows={3} value={content.banner?.subtitle} onChange={e => setContent({ ...content, banner: { ...content.banner, subtitle: e.target.value } })} className="admin-form-input font-bold text-black" placeholder="Browse handpicked packages..." />
                            </div>
                        </div>
                        <div>
